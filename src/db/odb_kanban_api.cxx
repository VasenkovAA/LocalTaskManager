...
namespace
{
  unsigned long to_ul(core::Id id)
  {
    if (id > static_cast<core::Id>(std::numeric_limits<unsigned long>::max()))
      throw std::runtime_error("Id does not fit into unsigned long (platform mismatch)");
    return static_cast<unsigned long>(id);
  }

  core::Id to_id(unsigned long id)
  {
    return static_cast<core::Id>(id);
  }
...
namespace db
{
  ...
  core::Id OdbKanbanApi::createBoard(const std::string& name, const std::string& description)
  {
    if (name.empty())
  ...
  void OdbKanbanApi::deleteColumn(core::Id columnId)
  {
    ...
    {
      throw std::runtime_error(std::string("deleteColumn: failed to load column: ") + e.what());
    }
  ...
  std::vector<core::TaskDto> OdbKanbanApi::listTasksForBoard(core::Id boardId, bool includeArchived)
  {
    const unsigned long bid = to_ul(boardId);
  ...
  std::vector<core::TaskDto> OdbKanbanApi::listTasksForColumn(core::Id columnId, bool includeArchived)
  {
    const unsigned long cid = to_ul(columnId);
  ...
  core::Id OdbKanbanApi::createTask(core::Id boardId,
                                   core::Id columnId,
  ...
  void OdbKanbanApi::setTasksOrderForColumn(core::Id columnId, const std::vector<core::Id>& orderedTaskIds)
  {
    const unsigned long cid = to_ul(columnId);
    ...
    for (std::size_t i = 0; i < orderedTaskIds.size(); ++i)
    {
      ...
      {
        throw std::runtime_error(std::string("setTasksOrderForColumn: failed to load task: ") + e.what());
      }
