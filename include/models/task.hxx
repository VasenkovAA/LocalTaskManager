...
public:
  ...
  Task(unsigned long board_id, unsigned long column_id, std::string title,
       std::string description = {}, int sort_order = 0)
      : board_id_(board_id), column_id_(column_id), title_(std::move(title)),
        description_(std::move(description)), sort_order_(sort_order) {}
