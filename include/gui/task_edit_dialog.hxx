...
public:
  ...
  static std::optional<TaskSnapshot> editTask(std::shared_ptr<odb::sqlite::database> db,
...
private:
  ...
  // Новый подход:
  //  - reloadCategoriesNoTx() вызывается ТОЛЬКО когда транзакция уже открыта снаружи
  //  - reloadCategoriesTx() сам открывает транзакцию (для кнопки "Add category")
